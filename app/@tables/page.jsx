...
import Design from "../@components/popup";
...
const table = () => {
  ...
  const [fixedVariants, setFixedVariants] = useState([
    "Primary Variant",
    "Variant 2",
  ]);
  ...
  const addColumn = (event) => {
    ...
    const newRows = [...rows];
    for (let i = 0; i < newRows.length; i++) newRows[i].Variants.push("");
  ...
  const activeFilter = (event, r, c) => {
    event.preventDefault();
    const newRow = [...rows];
    newRow[r].ProductFilters[c].tagStatus =
      !newRow[r].ProductFilters[c].tagStatus;
    setRows(newRow);
  };
  return (
    <div className="overflow-x-scroll">
          ...
          {rows.map((detail, index) => (
            ...
            >
                  ...
                  <div className="flex flex-wrap justify-center h-min">
                    {detail.ProductFilters.length !== 0 ? (
                      detail.ProductFilters.map((tag, index1) => (
                        <button
                          className={`rounded-md inline m-1 px-2 font-medium text-sm border ${
                            tag.tagStatus
                              ? "bg-prim-green-50 text-prim-green-100 border-prim-green-100"
                              : "bg-white text-prim-dark-200 shadow-sm border-slate-200"
                          }`}
                          key={index1 + 4000}
                          onClick={(event) =>
                            activeFilter(event, index, index1)
                          }
                        >
                      ...
                      <div>
                        <BTN
                          label={"Add Product Filters"}
                          toggle={setShowFilterTags}
                          r={index}
                          design={false}
                          rowIndexfn={setRowIndex}
                        />
                      </div>
                    )}
                  </div>
              ...
              {detail.Variants.map((variant, index2) => (
                <td>
                  <div className="flex flex-col w-100 bg-white m-4 p-3 min-h-48 w-48 rounded-md shadow-sm items-center justify-center">
                    <span key={index2 + 5000}></span>
                    {variant.length === 0 ? (
                      <div>
                        <BTN
                          label={"Add Design"}
                          toggle={setShowDesign}
                          r={index}
                          c={index2}
                          rowIndexfn={setRowIndex}
                          colmIndexfn={setColmIndex}
                        />
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-4">
                        <div className="relative">
                          ...
                          <button
                            className="absolute bottom-7 left-7 flex size-8 m-4 p-2 bg-white 
      ...
      {showFilterTags && (
        <Design
          showImages={false}
          showCatalouge={setShowFilterTags}
          data={rows}
          r={rowIndex}
          c={colmIndex}
          update={setRows}
